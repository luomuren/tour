	...
	btn.onmousedown = function (e) {
		...
		document.onmouseup = function (e) {
			...
			if (objX < slideBoxWidth) {
				...
				objX = slideBoxWidth;
				locked = true;
				$('#slide_box').html('验证通过<div id="btn"><i class="iconfont icon-xuanzhong" style="color: #fff;"></i></div>');
				$('#slide_box').css('color','#008000');
				$("#slide_box").css("borderColor", "#008000")
				$('#btn').html('<i class="iconfont icon-duihao" style="color:#008000"></i>');
			}
			$('#slide_xbox').width(objX + 'px');
	...
	cont.on({//绑定事件
		...
		touchend: function (e) {
			...
			if (objX < slideBoxWidth) {
				...
				$('#btn').css('background-image','url(./wancheng.png)')
			}
			$('#slide_xbox').width(objX + 'px');
		}
	});
